...
export function getPoemsBySlug(slug: string, fields: string[] = []) {
  const realSlug = slug.replace(/\.md$/, '')
...
export function getAllPoems(fields: string[] = []) {
  const slugs = getPoemsSlugs()
  const posts = slugs
    .map((slug) => getPoemsBySlug(slug, fields))
