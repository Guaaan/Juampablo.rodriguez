    ...
    >
      ...
      <div className="absolute inset-0 opacity-30">
        <div className="absolute inset-0" style={{
          backgroundImage: 'radial-gradient(circle, rgba(51,51,51,0.15) 1px, transparent 1px)',
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
        ...
        <div className="relative">
          <div className="w-[500px] h-[500px] rounded-full border-2 border-accent-7/15 border-dashed animate-spin-slow"
          ...
          {[0, 90, 180, 270].map((angle, i) => (
            <div
              key={i}
              className="absolute w-3 h-3 bg-accent-7/30 rounded-full"
              style={{
                left: '50%',
                top: '50%',
              ...
              >
                ...
                <div className={`relative transition-all duration-500 ${
                  ...
                  {!isHovered && !isSelected ? (
                    // Vista compacta
                    <div className="w-full h-full rounded-full bg-slateCustom shadow-sm flex items-center justify-center">
                    ...
                    // Vista expandida
                    <div className="w-full h-full rounded-3xl bg-white border-2 border-primary shadow-lg overflow-hidden">
                      {/* Cover Image */}
                      {post.coverImage && (
                        ...
                        {post.tags && post.tags.length > 0 && (
                          <div className="flex flex-wrap gap-2">
                            {post.tags.slice(0, 3).map(tag => (
                              <span
                                key={tag}
                                className="px-2 py-1 bg-white/20 rounded-full text-white text-xs font-semibold"
                              >
                                #{tag}
                              </span>
                            ))}
                ...
                {(isHovered || isSelected) && (
                  <div
                    className="absolute top-1/2 left-1/2 h-0.5 bg-accent-7/15 origin-left"
                    style={{
                      width: '250px',
          ...
          {/* Centro logo/info */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            <div className="w-32 h-32 rounded-full bg-white border-2 border-accent-7/10 flex items-center justify-center shadow-sm">
              <div className="text-center">
                <div className="text-black text-3xl font-black">
                  {sortedPosts.length}
                </div>
                <div className="text-accent-7/60 text-xs font-semibold">
                  POSTS
                </div>
              </div>
            </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 text-center">
        <div className="bg-white border border-accent-7/10 shadow-sm px-6 py-3 rounded-full">
          <p className="text-accent-7/70 text-sm">
